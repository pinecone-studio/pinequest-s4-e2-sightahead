...
// Cache translations so repeated lines (intros, recurring phrases) and
// re-observed lines don't hit the backend twice. Keyed by the source text.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Route 3: Translate a single live caption line (DOM-observer flow).
  if (message.type === "TRANSLATE_LINE") {
    handleTranslateLine(message.text)
      .then((translated) =>
...
// ── LIVE LINE TRANSLATION ───────────────────────────────────
// One caption line in → Mongolian out, via the backend's /translate.
// Runs in the worker so there's no CORS and the call leaves from the
// user's residential IP (the backend never touches YouTube).
async function handleTranslateLine(text) {
  const key = (text || "").trim();
  if (!key) return "";
  ...
  const response = await fetch(`${BACKEND_URL}/translate`, {
    ...
    body: JSON.stringify({ text: key, source_lang: "en", target_lang: "mn" }),
  });
  ...
  if (translated) translationCache.set(key, translated);
  return translated;
}
...
async function handleFetchCaptions(videoId) {
  ...
  if (!captionsMatch) {
    throw new Error(
      "No captions found for this video. It might not have subtitles.",
    );
  }
  ...
  if (manualTrack) {
    consoele.log("track found");
  }
  const englishTrack = tracks.find((t) => t.languageCode === "en");
  const selectedTrack = manualTrack || englishTrack || tracks[0];

  console.log(
    `[SightAhead] Using caption track: ${selectedTrack.name?.simpleText || selectedTrack.languageCode}`,
  );
...
async function handleTranslateAndDub(captions, videoId) {
  console.log(
    `[SightAhead] Sending ${captions.length} segments to backend for dubbing`,
  );
